...
GuillotineBinPack::GuillotineBinPack(double width, double height)
{
	Init(width, height);
}

void GuillotineBinPack::Init(double width, double height)
{
	binWidth = width;
	binHeight = height;
...
void CountNumFitting(const Rect &freeRect, double width, double height, const std::vector<RectSize> &rects, 
...
Rect GuillotineBinPack::Insert(double width, double height,int rotate,int id, bool merge, FreeRectChoiceHeuristic rectChoice,
	GuillotineSplitHeuristic splitMethod)
{
	// Find where to put the new rectangle.
	int freeNodeIndex = 0;
	Rect newRect = FindPositionForNewNode(width, height,rotate, rectChoice, &freeNodeIndex);
...
int GuillotineBinPack::ScoreByHeuristic(double width, double height, const Rect &freeRect, FreeRectChoiceHeuristic rectChoice)
{
	switch(rectChoice)
	{
...
int GuillotineBinPack::ScoreBestAreaFit(double width, double height, const Rect &freeRect)
{
	return freeRect.width * freeRect.height - width * height;
}

int GuillotineBinPack::ScoreBestShortSideFit(double width, double height, const Rect &freeRect)
{
...
int GuillotineBinPack::ScoreBestLongSideFit(double width, double height, const Rect &freeRect)
{
	int leftoverHoriz = abs(freeRect.width - width);
	int leftoverVert = abs(freeRect.height - height);
...
int GuillotineBinPack::ScoreWorstAreaFit(double width, double height, const Rect &freeRect)
{
	return -ScoreBestAreaFit(width, height, freeRect);
}

int GuillotineBinPack::ScoreWorstShortSideFit(double width, double height, const Rect &freeRect)
{
	return -ScoreBestShortSideFit(width, height, freeRect);
}

int GuillotineBinPack::ScoreWorstLongSideFit(double width, double height, const Rect &freeRect)
{
	return -ScoreBestLongSideFit(width, height, freeRect);
}

Rect GuillotineBinPack::FindPositionForNewNode(double width, double height,int rotate, FreeRectChoiceHeuristic rectChoice, int *nodeIndex)
{
	Rect bestNode;
	...
	for(size_t i = 0; i < freeRectangles.size(); ++i)
	{
		...
		else if (rotate==1&&height == freeRectangles[i].width && width == freeRectangles[i].height)
		{
			bestNode.x = freeRectangles[i].x;
			bestNode.y = freeRectangles[i].y;
...
double GuillotineBinPack::score(double width, double height,int rotate, FreeRectChoiceHeuristic rectChoice)
{
...
void GuillotineBinPack::SplitFreeRectAlongAxis(const Rect &freeRect, const Rect &placedRect, bool splitHorizontal)
{
	...
	if (bottom.width > 0.0001 && bottom.height > 0.0001)
    {
        num_qiege+=1;
        freeRectangles.push_back(bottom);
    }
	if (right.width > 0.0001 && right.height > 0.0001){
        num_qiege+=1;
