...
inline std::ostream& operator<<(std::ostream& os, const LongDouble& c) {
	os << toLongDouble(c);
	return os;
}

inline std::istream& operator>>(std::istream& is, LongDouble& c) {
	long double val;
	is >> val;
	c.setVal(val);
	return is;
...
namespace libnfporb {
inline bool equals(const LongDouble& lhs, const LongDouble& rhs) {
	if (lhs.val() == rhs.val())
		return true;
...
inline bool equals(const point_t& lhs, const point_t& rhs) {
	return equals(lhs.x_, rhs.x_) && equals(lhs.y_, rhs.y_);
}

inline bool equals(const segment_t& lhs, const segment_t& rhs) {
	return equals(lhs.first, rhs.first) && equals(lhs.second, rhs.second);
}
inline std::ostream& operator<<(std::ostream& os, const segment_t& seg) {
...
inline point_t normalize(const point_t& vec) {
	point_t norm = vec;
...
inline point_t flip(const point_t& vec) {
	point_t flipped = vec;
...
inline bool is_parallel(const segment_t& s1, const segment_t& s2) {
...
inline Alignment get_alignment(const segment_t& seg, const point_t& pt) {
...
inline long double get_inner_angle(const point_t& joint, const point_t& end1, const point_t& end2) {
...
inline std::vector<psize_t> find_minimum_x(const polygon_t& p) {
...
inline std::vector<psize_t> find_maximum_x(const polygon_t& p) {
	std::vector<psize_t> result;
...
inline std::vector<psize_t> find_minimum_y(const polygon_t& p) {
	std::vector<psize_t> result;
	coord_t min = MAX_COORD;
	auto& po = p.outer();
	for (psize_t i = 0; i < p.outer().size() - 1; ++i) {
...
inline std::vector<psize_t> find_maximum_y(const polygon_t& p) {
	std::vector<psize_t> result;
	coord_t max = MIN_COORD;
	auto& po = p.outer();
	for (psize_t i = 0; i < p.outer().size() - 1; ++i) {
...
inline psize_t find_point(const polygon_t::ring_type& ring, const point_t& pt) {
 ...
 */
inline bool in_nfp(const point_t& pt, const nfp_t& nfp) {
